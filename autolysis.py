...
def extract_relevant_data(df, max_rows=100):
    ...
    categorical_data = df.select_dtypes(exclude=["number"]).sample(
        n=min(len(df), max_rows), random_state=42
    )
    return {
...
def prepare_function_data(df, function_name):
    ...
    if function_name == "outlier_detection" or function_name == "correlation_analysis":
        return (
            df.select_dtypes(include=["number"]).head(100).to_dict(orient="records")
        )  # Numeric data sample
    elif function_name == "time_series_anomalies":
        if (
            "timestamp" in df.columns
            and len(df.select_dtypes(include=["number"]).columns) > 0
        ):
            return {
...
def request_llm_analysis(metadata, data_samples, tool):
    ...
    try:
        ...
        if response.status_code == 200:
            return response.json()["choices"][0]["message"]["tool_calls"][0][
                "function"
            ]["arguments"]
        else:
            print(f"Error: {response.status_code} - {response.text}")
...
def visualize_outliers(outliers):
    ...
    fig, ax = plt.subplots(
        figsize=(6.4, 6.4)
    )  # Original size to match the 512x512 px requirement after saving
...
def visualize_correlation_matrix(correlation_matrix):
    ...
    fig, ax = plt.subplots(
        figsize=(20, 18)
    )  # Adjust dimensions to handle many features
            ...
            if relevant_data:
                ...
                if result:
                    ...
                    if tool["name"] == "outlier_detection":
                        ...
                        chart_paths.append(
                            save_chart(chart, directory_name, "outliers.png")
                        )
